...
func main() {
	username := "admin"
	token := "d5fbd378f2a64aa19ecc51204ec74831619c3fb5"
	filehash := "0d279f151aefeb4f26f38fe6736071ae85996951"

	// 1. 请求初始化分块上传接口
	resp, err := http.PostForm(
		"http://192.168.105.9:8080/file/mpupload/init",
		url.Values{
			"username": {username},
			"token":    {token},
			"filehash": {filehash},
	...
	// 3. 请求分块上传接口
	filename := "/home/ubuntu/share/navicat150_mysql_cs.dmg"
	tURL := "http://192.168.105.9:8080/file/mpupload/uppart?" +
		"username=admin&token=" + token + "&uploadid=" + uploadID
	multipartUpload(filename, tURL, chunkSize)

	// 4. 请求分块完成接口
	resp, err = http.PostForm(
		"http://192.168.105.9:8080/file/mpupload/complete",
		url.Values{
			"username": {username},
			"token":    {token},
			"filehash": {filehash},
			"filesize": {"17357361"},
			"filename": {"navicat150_mysql_cs.dmg"},
			"uploadid": {uploadID},
		})
